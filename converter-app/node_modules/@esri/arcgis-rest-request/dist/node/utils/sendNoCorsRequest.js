...
exports.isNoCorsRequestRequired = exports.isNoCorsDomain = exports.registerNoCorsDomains = exports.getRegisteredNoCorsDomains = exports.sendNoCorsRequest = void 0;
...
function sendNoCorsRequest(url) {
    ...
    if (requestConfig_1.requestConfig.pendingNoCorsRequests[origin]) {
        return requestConfig_1.requestConfig.pendingNoCorsRequests[origin];
    }
    // Make the request and add to the cache
    requestConfig_1.requestConfig.pendingNoCorsRequests[origin] = fetch(url, {
        mode: "no-cors",
        credentials: "include",
    })
        ...
        if (requestConfig_1.requestConfig.noCorsDomains.indexOf(origin) === -1) {
            requestConfig_1.requestConfig.noCorsDomains.push(origin);
        }
        // Hold the timestamp of this request so we can decide when to
        // send another request to this domain
        requestConfig_1.requestConfig.crossOriginNoCorsDomains[origin.toLowerCase()] = Date.now();
        // Remove the pending request from the cache
        delete requestConfig_1.requestConfig.pendingNoCorsRequests[origin];
...
function getRegisteredNoCorsDomains() {
    ...
    return requestConfig_1.requestConfig.noCorsDomains;
}
...
function registerNoCorsDomains(authorizedCrossOriginNoCorsDomains) {
    ...
    authorizedCrossOriginNoCorsDomains.forEach(function (domain) {
...
function addNoCorsDomain(url) {
    ...
    if (requestConfig_1.requestConfig.noCorsDomains.indexOf(domain) === -1) {
        requestConfig_1.requestConfig.noCorsDomains.push(domain);
    }
}
/**
...
function isNoCorsDomain(url) {
    var result = false;
    if (requestConfig_1.requestConfig.noCorsDomains.length) {
        // is the current url in the no-cors domains?
        var origin_1 = new URL(url).origin.toLowerCase();
        result = requestConfig_1.requestConfig.noCorsDomains.some(function (domain) {
            return origin_1.includes(domain);
        });
    }
    return result;
}
...
function isNoCorsRequestRequired(url) {
    ...
    if (isNoCorsDomain(url)) {
        var origin_2 = new URL(url).origin.toLowerCase();
        // check if we have sent a no-cors request to this domain in the last hour
        var lastRequest = requestConfig_1.requestConfig.crossOriginNoCorsDomains[origin_2] || 0;
