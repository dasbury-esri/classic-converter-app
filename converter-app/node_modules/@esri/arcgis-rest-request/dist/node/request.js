...
var sendNoCorsRequest_1 = require("./utils/sendNoCorsRequest");
var isSameOrigin_1 = require("./utils/isSameOrigin");
...
function request(url, requestOptions) {
    ...
    if (sendNoCorsRequest_1.isNoCorsDomain(url)) {
    ...
    if (typeof window !== "undefined") {
        sameOrigin = isSameOrigin_1.isSameOrigin(url);
    }
    var requiresNoCors = !sameOrigin && sendNoCorsRequest_1.isNoCorsRequestRequired(url);
    ...
    var firstPromise = Promise.resolve();
    if (requiresNoCors) {
        // ensure we send cookies on the request after
        fetchOptions.credentials = "include";
        firstPromise = sendNoCorsRequest_1.sendNoCorsRequest(url);
    }
    return firstPromise
        .then(function () {
        if (authentication) {
        ...
        if ((params.f === "json" || params.f === "geojson") && !rawResponse) {
            ...
            if (data && /\/sharing\/rest\/(accounts|portals)\/self/i.test(url)) {
                ...
                if (Array.isArray(data.authorizedCrossOriginNoCorsDomains)) {
                    sendNoCorsRequest_1.registerNoCorsDomains(data.authorizedCrossOriginNoCorsDomains);
                }
            }
            if (originalAuthError) {
